...
# Google Books queries that reliably surface series; run concurrently
GOOGLE_SERIES_QUERIES = [
    'subject:fantasy series', 'subject:"science fiction" series', 'subject:mystery series',
    'subject:thriller series', 'subject:romance series', 'subject:"young adult" series',
    'subject:"historical fiction" series', 'subject:dystopian series',
    'subject:"juvenile fiction" series', 'subject:"graphic novels" series',
    'intitle:"book 1"', 'intitle:trilogy', 'intitle:chronicles', 'intitle:saga',
]
...
class SeriesRecommender:
    ...
    def fetch_google_query(self, query: str) -> List[Dict[str, Any]]:
        """Fetch one Google Books query and keep the results that look like series books."""
        books = []
        try:
            response = requests.get(
    ...
    def fetch_google_series(self) -> List[Dict[str, Any]]:
        """Fetch series books from Google Books API, running the queries concurrently."""
        with ThreadPoolExecutor(max_workers=8) as executor:
            results = executor.map(self.fetch_google_query, GOOGLE_SERIES_QUERIES)
    ...
    def get_recommendations(self) -> List[Dict[str, Any]]:
        ...
        # Combine all books
        all_books = nyt_books + google_books
